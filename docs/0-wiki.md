...
|||**2026-08-29 (T5.1 Define lifecycle and contribution contract)**: Thêm `docs/design-system/DESIGN.md` §10 — component/token lifecycle (`experimental` → `stable` → `deprecated` → `removed`), contribution checklist (reuse search, gap evidence, owner, showcase, tests, audit, ADR decision), breaking-change/deprecation rules (migration note, deprecation window, consumer proof), ADR usage guidance, và example proposal dry-run (`Pagination`). Cập nhật `docs/design-system/ROADMAP.md` §4.2/§6.2/Phase 5 để phản ánh governance đã có; cập nhật `tasks/plan-design-system-v2.md`, `tasks/todo-design-system-v2.md`. Verify: `npx tsc --noEmit` pass, `npm run test:unit:design-system` pass (84/476), `npm run build` pass.||
...
||||**2026-08-29 (T4.3 Slice 2 — Productize Search → result pattern)**: Tạo `src/entrypoints/design-system-showcase/pages/PatternSearchResult.showcase.tsx` + `.module.css` minh họa flow search: `SearchField` + `Tabs` filter (All/English/Japanese) + loading skeleton/empty/error/result list dùng `Card` với `role="option"` trong `VStack` `role="listbox"`. Thêm `e2e/showcase-patterns-search-result.spec.ts` (5 tests: search results, tab filter, empty, error, keyboard Enter, axe WCAG 2.1 AA). Chạy Playwright `showcase` project: 5/5 passed. Cập nhật `docs/2-architechture-system.md`, `docs/0-wiki.md`, `tasks/plan-design-system-v2.md`, `tasks/todo-design-system-v2.md`, `COMPONENT_INVENTORY`. Verify: `npx tsc --noEmit` pass, `npm run test:unit:design-system` pass (84/476), `npx playwright test e2e/showcase-patterns-search-result.spec.ts --project=showcase` 5 passed, `npm run build` pass, design-system-guardian audit PASS.||

||||**2026-08-29 (T4.3 Slice 3 — Productize Form submit pattern)**: Tạo `src/entrypoints/design-system-showcase/pages/PatternFormSubmit.showcase.tsx` + `.module.css` minh họa form validation (`InputField`, `Select` + `Label`), trạng thái `submitting`/`success`/`error`, `Dialog` xác nhận destructive action. Thêm `e2e/showcase-patterns-form-submit.spec.ts` (5 tests: validation, success, error, dialog cancel/confirm, axe WCAG 2.1 AA). Chạy Playwright `showcase` project: 5/5 passed. Cập nhật `docs/2-architechture-system.md`, `docs/0-wiki.md`, `tasks/plan-design-system-v2.md`, `tasks/todo-design-system-v2.md`, `COMPONENT_INVENTORY`. Verify: `npx tsc --noEmit` pass, `npm run test:unit:design-system` pass (84/476), `npx playwright test e2e/showcase-patterns-form-submit.spec.ts --project=showcase` 5 passed, `npm run build` pass, design-system-guardian audit PASS.||

||||**2026-08-29 (T4.3 Slice 4 — Productize Subtitle acquisition pattern)**: Tạo `src/entrypoints/design-system-showcase/pages/PatternSubtitleAcquisition.showcase.tsx` + `.module.css` minh họa flow subtitle: nhập URL → detect → list candidates (`VStack role="listbox"`, `Card role="option"` với `StatusDot`) → acquire. Thêm `e2e/showcase-patterns-subtitle-acquisition.spec.ts` (5 tests: URL validation, detect/list, acquire, empty/error, keyboard Enter, axe WCAG 2.1 AA). Chạy Playwright `showcase` project: 5/5 passed. Cập nhật `docs/2-architechture-system.md`, `docs/0-wiki.md`, `tasks/plan-design-system-v2.md`, `tasks/todo-design-system-v2.md`, `COMPONENT_INVENTORY`. Verify: `npx tsc --noEmit` pass, `npm run test:unit:design-system` pass (84/476), `npx playwright test e2e/showcase-patterns-subtitle-acquisition.spec.ts --project=showcase` 5 passed, `npm run build` pass, design-system-guardian audit PASS.||

||||**2026-08-29 (T4.3 Slice 5 — Productize Vocabulary capture pattern)**: Tạo `src/entrypoints/design-system-showcase/pages/PatternVocabularyCapture.showcase.tsx` + `.module.css` minh họa click từ trong phụ đề (token `role="button"` có `tabIndex`), `Dialog` lookup, `Select` status, capture vào list với `StatusDot` + `Text`. Xử lý contrast `color-text-secondary` trên nền sáng bằng cách dùng `Text` mặc định thay vì `color="secondary"` và loại `Badge variant="warning"` khỏi list. Thêm `e2e/showcase-patterns-vocabulary-capture.spec.ts` (5 tests: word dialog, capture, cancel, lookup error, keyboard, axe WCAG 2.1 AA). Chạy Playwright `showcase` project: 5/5 passed. Cập nhật `docs/2-architechture-system.md`, `docs/0-wiki.md`, `tasks/plan-design-system-v2.md`, `tasks/todo-design-system-v2.md`, `COMPONENT_INVENTORY`. Verify: `npx tsc --noEmit` pass, `npm run test:unit:design-system` pass (84/476), `npx playwright test e2e/showcase-patterns-vocabulary-capture.spec.ts --project=showcase` 5 passed, `npm run build` pass, design-system-guardian audit PASS.||

||||**2026-08-29 (T4.3 Slice 1 — Productize Async states pattern)**: Tạo `docs/design-system/PATTERN_CATALOG.md` với 7 pattern đã được chứng minh từ production flows (async states, search→result, form submit, subtitle acquisition, vocabulary capture, loading, empty). Tạo showcase `src/entrypoints/design-system-showcase/pages/PatternAsyncStates.showcase.tsx` + `.module.css` minh họa idle/loading/empty/error/success bằng Spinner, Skeleton, EmptyState, Alert, Card, Button, StatusDot. Thêm `e2e/showcase-patterns-async-states.spec.ts` (5 tests: loading/empty/error/success/keyboard, axe WCAG 2.1 AA cho mỗi state). Chạy Playwright `showcase` project: 5/5 passed. Cập nhật `docs/2-architechture-system.md` (design-system showcase index), `tasks/todo-design-system-v2.md`, `tasks/plan-design-system-v2.md`. Verify: `npx tsc --noEmit` pass, `npm run test:unit:design-system` pass (84/476), `npx playwright test e2e/showcase-patterns-async-states.spec.ts --project=showcase` 5 passed, `npm run build` pass, design-system-guardian audit PASS.||

||||**2026-08-29 (T4.2 Slice 3 — Close evidence gaps for Breadcrumb/Tree)**: Thêm behavior test cho `src/shared/ui/Breadcrumb.test.tsx` (render items, default current last item, explicit current, empty) và `src/shared/ui/Tree.test.tsx` (tree role/label, expanded children, click toggle+select, Enter/Space keyboard, active aria-selected). Chạy `npm run generate:component-inventory`; `missingTest` rỗng, tất cả public export đều có test. `Breadcrumb` và `Tree` chuyển từ `needs-test` sang `unused` (có showcase + test nhưng 0 production consumer). Cập nhật `tasks/todo-design-system-v2.md`, `tasks/plan-design-system-v2.md`. Verify: `npx tsc --noEmit` pass, `npm run test:unit:design-system` pass (84/476), `npm run build` pass.||

||||**2026-08-29 (T4.2 Slice 2 — Close evidence gaps for SliderRow/SearchableSelect/HintIcon)**: Thêm behavior test cho 3 public export có consumer: `src/shared/ui/SliderRow.test.tsx` (label/value render, range slider, onChange, disabled, disabledNote, bubble variant, formatValue), `src/shared/ui/SearchableSelect.test.tsx` (trigger, open/close, select, filter, empty state, Escape, disabled), `src/shared/ui/HintIcon.test.tsx` (button aria-label, toggle, Escape, click outside). Sửa `src/shared/ui/SearchableSelect.tsx`: bỏ `role="listbox"` trùng lặp trên `<ul>` bên trong menu `role="listbox"` để tránh nested listbox và test query lỗi. Chạy lại `npm run generate:component-inventory`; SliderRow/SearchableSelect/HintIcon chuyển sang stable, missingTest chỉ còn `Breadcrumb` và `Tree` (cả hai đều 0 consumer). Cập nhật `tasks/todo-design-system-v2.md`, `tasks/plan-design-system-v2.md`. Verify: `npx tsc --noEmit` pass, `npm run test:unit:design-system` pass (82/467), `npm run build` pass.||
