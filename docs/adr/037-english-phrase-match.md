...
5. ✅ Add compact blob loader and worker anchor index (`phraseIndexLoader.ts` + `lookupWorkerHandler.ts`): validate magic/compilerVersion/termCount/anchor bounds; hydrate into `ResidentPhraseIndex` with anchor→templateIDs map; warm lookup via `resolveCandidatesByAnchors` (never scans all templates); malformed blobs fail closed (error recorded, lookup skips the resource). Worker `LOOKUP` now runs `matchPhrase` against resident indexes and returns `detectedPhrase` on hit.
6. ✅ Add bounded DP matcher + deterministic ranking (`phraseMatcher.ts`); 36 tests covering P01-P33 positive (P02/P07/P14/P19/P21/P22/P29b/P30/P31 added), N01-N20 negative (N04/N05/N12/N14/N20 added), ranking, edge cases. Candidate cap uses anchor-overlap scoring (top-256 by window-anchor count) instead of aborting — fixes the 34k-fixture false-negative where 'the' (1137 postings) caused premature abort. Fixture strict test (`phraseMatcher.fixture.test.ts`) verifies real Cambridge fixture: ≥1000 templates, <8MB blob, round-trip, every template has ≥1 anchor, every anchor resolves, representative matches, no false positives, sourceResourceId preserved.
...
9. ✅ Run fixture benchmark (`phraseMatchBenchmark.test.ts`): compile 34k terms 80ms, blob <8MB, deserialize <200ms, match <30ms (34k-fixture with high-frequency anchors), tokenize 50 words 0.04ms.
