...
| `MediaList` | `features/cardCreator/ui/MediaList.tsx` | (files, kind, addLabel, onAdd, onRemove, onFilesDrop?, addDisabled?, testId?) → ReactElement | CardCreatorDialogContent | Image gallery (horizontal 120px thumbnails + add button + preview overlay) or audio list (vertical rows + play/remove + empty dropzone); supports drag-and-drop file add |
