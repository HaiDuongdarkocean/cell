...
|| `features/dictionaryPopup/ui/CandidateView.tsx` | shared/ui, icons, rankToBand, config, useCandidate, DictionaryPanelView.module.css | DictionaryPanelView | React card: candidate header, toolbar tabs, lazy media panels, definitions; uses useCandidate (per-candidate hook) |
...
|| `features/dictionaryPopup/ui/DictionaryPanelView.tsx` | shared/ui, useDictionaryPanel, searchHistory, CandidateView | DictionaryTab | React view: search input, bounded history, candidate chip bar with jump-scroll, scrollable CandidateView list (no global tab state) |
