...
│   │       └── ui/SubtitlePanels.tsx + .module.css  # T045: shared shadow-root container composing SubtitleBlock, NavCluster, SubtitleManagerPanel, SubtitleOffsetPanel, SubtitleToast, SubtitleHint; exposes imperative controller via ref; Player Mode toggle (player-mode-btn replaces generate-native-btn in secondaryCol; generate-native-btn moved to extraCol)
│   │       └── ui/PlayerModeOverlay.tsx + .module.css  # Player Mode: fixed full-viewport overlay (VideoStage → ContentOther → PlayerActionDock); transparent/click-through outside interactive dock so host video remains visible; reuses SubtitleBlock + original two-column NavCluster/action cluster composition inside the dock; Escape exits via window listener; dock always visible; z-index --z-overlay-settings (below dictionary popup)
...
| `features/subtitle/ui/subtitleShortcuts.ts` | types (KeyboardShortcut) | contentScriptController.ts | Keyboard handler: handleShortcutKey (pure, guard input/textarea). **ADR-027**: default `generate-native` action (key `g`); default `play-pause` action (key `pause`) |
...
| `handleShortcutKey` | `features/subtitle/ui/subtitleShortcuts.ts` | (string, KeyboardShortcut[], EventTarget) → ShortcutAction \| null | content-script.ts, **sidepanel/App.tsx** | Pure: map key → action, guard input/textarea focus. **ADR-027**: default `DEFAULT_KEYBOARD_SHORTCUTS` includes `generate-native` (key `g`) and `play-pause` (key `pause`) |
