...
| `createOverlayLayer` | `content/subtitleUI.ts` | (role, OverlayStyleConfig, HTMLElement) → {overlay, textSpan, dragHandle} | subtitleOverlay | **NEW (planned ADR-013)**: Create 1 overlay div độc lập (target OR native) + text span + drag handle. Refactor createOverlay (1 div 2 span) → 2 div độc lập |
| `applyStyle` | `content/subtitleUI.ts` | (OverlayStyleConfig, HTMLDivElement) → void | subtitleOverlay, content-script.ts | **NEW (planned ADR-013)**: Set inline style (fontSize, color, bg+alpha, opacity, textShadow, fontFamily, yOffset%, align, visible). Thin wrapper, logic ở pure function |
