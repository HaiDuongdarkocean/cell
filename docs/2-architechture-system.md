...
| `shared/ui/Button.tsx` + `.module.css` + `.style-guard.test.ts` + `.showcase.tsx` + `.showcase.module.css` | — | App.redesigned, ResourceCard, Dropzone, ImportProgress, DeleteConfirmModal, ResourcesPanel, ThemePanel, ThemeImportExport, SelectionBar, SubtitleManagerFooter, OcrSettingsPanel | **Liquid Glass button atom (v4)**: 6 action variants (primary/glass/outline/ghost/success/destructive) + aliases (secondary/primarySubtle/link), all sharing the same smoked-blue liquid-glass material with caustic rim highlights. 4 sizes (sm/md/lg/xl), horizontal/vertical orientations, 8 states incl. active toggle/loading/error, pill shape. Style guard enforces token contracts. |
