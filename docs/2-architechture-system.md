...
│   └── dictionaryPopup/  # Popup Dictionary (spec §9, ADR-037, ADR-045) — types, schema; worker/lookupWorker, lookupWorkerHandler, phraseIndexLoader, resourcePriority; logic/lruCache, lookupOrchestrator, lookupOrchestratorBenchmark, englishLemma (ADR-041: unified multi-candidate lemma); trigger/subtitleTriggerController, subtitleTokenWrap, webTextTriggerController; controller/webTextDictionaryController (per-tab shared lookup + popup + highlight + card-creator wiring for web text and subtitle tokens); services/wordStatusStore, wordStatusClient, frequencyClient, quickAddAssembler, quickAddHandler, sendToCreator, ttsEngineService; ui/popupShell (Shadow DOM popup: role=dialog, focus trap, drag header, toast overlay), popupContent (renderPopupContent, renderActiveEntry, renderHeader 2-row, renderFooter send/settings, renderDefinitions, renderCandidateChips, getOrCreateCandidatesContainer/getOrCreateMaterialsSlot), popupToolbar (renderToolbar with responsive labels, renderAudioPanel with play/pause state + TTS fallback, renderImagePanel, renderTranslatePanel, renderLinksPanel chips), popupDictionaryController (showPopup, appendCandidate, setActiveCandidate, active candidate state, single context-aware toolbar, playTermAudio (on-demand community fetch + headerAudioId tracking), playSentenceAudio, playUrlBestEffort, Quick Add toast feedback, status cycle with onStatusChange callback), wordHighlight (visual highlight for looked-up word: DOM mark + overlay fallback, token-aware); plugins/languagePlugin, englishPlugin, chinesePlugin, fallbackPlugin, pluginRegistry
│   └── tokenize/       # Tokenize on Media (ADR-047) — VDLT-Hybrid: types; logic/textTokenizer (+prepareTokenBlock synchronous tokenization), logic/tokenizeBlock (boundary detection + findTextBlocksInNodes for incremental SPA re-scans), logic/viewportTracker (IntersectionObserver wrapper), logic/tokenizeCache (LRU + touch + domMap cleanup on evict/delete), logic/tokenizeScheduler (viewport fast path + idle chunk + task error isolation); controller/webTokenizeController (lazy scan/observe only while enabled; viewport 300px buffer, incremental MutationObserver re-scan with requestAnimationFrame fast path + characterData handling, batched status/frequency metadata, eager tryBindVisible, early-bind then async rebind; orchestrates scheduler, cache, renderer, badge + Popup Dictionary wiring), controller/subtitleTokenizeController; services/tokenizeSettingsStore (per-domain enable state), services/tokenizeStateStore (selected terms + toggles); utils/frequencyBand (rank → high/medium/low/none); ui/tokenSpanRenderer (defensive bind/unbind token spans for SPA re-render), ui/tokenSpanCss (host-page token styles), ui/tokenBadge (FAB + mini panel), ui/tokenBadgeCss
...
| `lookupOrchestrator` | `features/dictionaryPopup/logic/lookupOrchestrator.ts` | (LookupRequest, deps?, signal?) → Promise<LookupResult> | lookupWorkerHandler | **spec §4.6.3/§9.4**: EN phrase match (ADR-037) → dict query; ZH FMM segment → dict query; fallback token → dict query; lemma fallback (ADR-041: multi-candidate inflectional morphology — "easiest"→"easy", "bigger"→"big", "children"→"child", "cat's"→"cat"); assembles LookupResult (definitions, reading, frequency, status, detectedPhrase, matchSource) |
...
| `englishLemma` | `features/dictionaryPopup/logic/englishLemma.ts` | (word: string) → string | englishPlugin.lemma, lookupOrchestrator (fallback) | **ADR-041**: single-candidate wrapper for `englishLemmaCandidates()[0]`; backward compat |
| `englishLemmaCandidates` | `features/dictionaryPopup/logic/englishLemma.ts` | (word: string) → string[] | englishPlugin.lemmaCandidates, phraseMatcher.candidateLemmas, lookupOrchestrator (fallback) | **ADR-041**: unified multi-candidate lemma — ALL 8 English inflectional suffixes + irregular verbs (~250), irregular comparison (better→good), irregular plural nouns (children→child), -ves plural (knives→knife), possessive -'s (cat's→cat); CVC doubling (bigger→big), silent-e (nicest→nice), ie→ying (lying→lie), sibilant -es (boxes→watch); false positives harmless — orchestrator tries raw term first |
...
| `SubtitleTriggerController` | `features/dictionaryPopup/trigger/subtitleTriggerController.ts` | class: attach/detach/setTriggerMode/cancelInFlight | contentScriptController | **spec §4.6.3 A2, §9.4, D9**: debounce 150ms hover / 50ms click, LOOKUP_CANCEL on new trigger, requestId routing; onLookup callback receives the token span for highlight; wraps subtitle text span into per-token spans (EN per-word, ZH per-segment) |
...
| `WebTextTriggerController` | `features/dictionaryPopup/trigger/webTextTriggerController.ts` | class: attach/detach/setTriggerMode/cancelInFlight | webTextDictionaryController | **spec §5.2 P1**: web text lookup — mouseup with selection → dispatch LookupRequest; empty selection falls back to caretRangeFromPoint (supports `user-select:none` sites); hover+modifier → caretRangeFromPoint word extract; builds a non-collapsed Range covering the word for highlight; skips `.js-cell-popup-host` and `.js-cell-token` |
| `WebTextDictionaryController` | `features/dictionaryPopup/controller/webTextDictionaryController.ts` | createWebTextDictionaryController(deps) → controller | content-script.ts, contentScriptController.ts | **ADR-046**: per-tab shared controller for web-text + subtitle dictionary popup. Owns `WordHighlight`, wires `WebTriggerController`, sends LOOKUP_REQUEST, renders popup via `popupDictionaryController` with `onStatusChange` applied to the source token, handles Quick Add / Send to Card, late-binds video/cues via `configureVideo` |
...
| `renderPopupContent` | `features/dictionaryPopup/ui/popupContent.ts` | (container, LookupResult, WordStatus, DefinitionSelection, PopupContentCallbacks) → void | popupDictionaryController | Clear container → active entry (header + toolbar slot + definitions) + candidates + footer |
...
| `renderCandidateChips` | `features/dictionaryPopup/ui/popupContent.ts` | (container, candidates, activeIdx, onChipClick) → void | popupDictionaryController | Horizontal scrollable chips only (v3); CSS truncation; active chip aria-current |
...
| `tokenizeTextBlock` | `features/tokenize/logic/textTokenizer.ts` | (text, langCode?) → Token[] | webTokenizeController, subtitleTokenizeController | Split text into word/punctuation tokens with UTF-16 offsets; EN uses phraseMatcher tokenizeSentence, ZH uses FMM fallback |
...
| `findTextBlocks` | `features/tokenize/logic/tokenizeBlock.ts` | (root, options?) → TokenBlock[] | webTokenizeController | Walk text nodes, skip forbidden tags/roles, build TokenBlock list; explicit `role="link"` remains readable/tokenizable (ponytail: does not merge adjacent inline text nodes) |
| `findTextBlocksInNodes` | `features/tokenize/logic/tokenizeBlock.ts` | (nodes, options?) → TokenBlock[] | webTokenizeController | Incremental scan of MutationObserver `addedNodes`; skips disconnected nodes and nodes inside existing token spans |
| `TokenizeScheduler` | `features/tokenize/logic/tokenizeScheduler.ts` | class (priority queue + idle fallback) | webTokenizeController, subtitleTokenizeController | Viewport tasks use next-tick fast path; buffer/idle tasks use idle budget; per-task try/catch so one failing bind/unbind cannot hang the queue; `flush()` drains all priorities for tests |
| `ViewportTracker` | `features/tokenize/logic/viewportTracker.ts` | class (IntersectionObserver wrapper) | webTokenizeController | observe/unobserve with onEnter/onExit handlers; no-op when IntersectionObserver missing (jsdom/tests). ADR-048: `observe` fires `onEnter` synchronously for new handler sets on already-intersecting elements (SPA re-render reuses parent → IO won't fire new callback) |
| `TokenizeCache` | `features/tokenize/logic/tokenizeCache.ts` | class (LRU + WeakMap<Element, TokenBlock>) | webTokenizeController | Bounded block cache; `onEvict` unbinds DOM and removes the block from `domMap`; `touch(block)` refreshes recency for visible/bound blocks; `getByElement` reverse lookup |
| `createWebTokenizeController` | `features/tokenize/controller/webTokenizeController.ts` | (options) → Promise<WebTokenizeController> | content-script.ts | Lazy scan/observe only while enabled; persisted enable waits for `window.load` + 500ms child-list DOM quiescence (HYDRATION_QUIET_MS) capped by MAX_ACTIVATION_DELAY_MS=3000ms hard deadline (ADR-048: heavy SPA never goes quiet); mutation re-scan accumulates `addedNodes` and uses `findTextBlocksInNodes` for incremental scans; fast path schedules `requestAnimationFrame` to bind new nodes immediately, with a 300ms trailing-debounce fallback capped by MAX_MUTATION_SCAN_DELAY_MS=1500ms; also observes `characterData` to re-tokenize in-place text changes; viewport buffer expanded to 300px; eager `tryBindVisible` binds blocks already in viewport without waiting for IntersectionObserver; visible blocks are bound immediately after synchronous tokenization, then status/frequency metadata is fetched in one batched background request and rebound; `blocks` array is pruned when cache evicts; cancels pending observer/listener on disable/destroy; disable disconnects observer, unbinds and clears blocks; setting changes rebind visible blocks; Popup Dictionary wiring |
| `createSubtitleTokenizeController` | `features/tokenize/controller/subtitleTokenizeController.ts` | (options) → SubtitleTokenizeController | subtitleBlockController.ts | Tokenize active + windowed subtitle cues; status/frequency classes; keyboard shortcuts; Popup Dictionary wiring via `onOpenDictionary` |
| `rankToBand` | `features/tokenize/utils/frequencyBand.ts` | (rank) → TokenFrequencyBand | webTokenizeController, subtitleTokenizeController | Rank thresholds: ≤1000 high, ≤5000 medium, ≤20000 low, else none (ponytail: arbitrary cutoffs, tune against corpus) |
| `entriesToBand` | `features/tokenize/utils/frequencyBand.ts` | (FrequencyEntry[]) → TokenFrequencyBand | webTokenizeController, subtitleTokenizeController | Pick best (lowest) rank across resources |
...
| `createTokenBadge` | `features/tokenize/ui/tokenBadge.ts` | (options) → TokenBadge | webTokenizeController | Floating FAB + Shadow DOM mini panel with enable/status/frequency toggles and dictionary button. Theme-aware: sets `data-theme` on panel inside shadow tree, listens to `chrome.storage.onChanged` + `matchMedia` for real-time light/dark switch. Reuses `.btn--primary` / `.icon-btn--xs` from `components.css` (SSOT hover/active). Toggle uses DS pattern (`<button aria-pressed>` + `.cell-toggle__thumb`). FAB is drag-to-reposition (pointer events, 4px click/drag threshold, position persisted to `STORAGE_KEYS.TOKEN_BADGE_POSITION`); panel anchors to FAB rect on open and follows during drag. Click-outside (`document` `pointerdown` capture listener) closes the open panel. |
