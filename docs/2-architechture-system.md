...
| `renderCandidateList` | `features/dictionaryPopup/ui/popupContent.ts` | (container, candidates, activeIdx, onSelect) → void | popupDictionaryController | Vertical list below chips; each item shows term + reading; click calls `onSelect(idx)` to switch active candidate |
