...
| `features/ocr/overlay/regionSelector.ts` | tokens (STATIC_TOKENS), icons (checkIcon, xIcon), ocrStateTypes (CustomRegion) | ocrContentScript.ts (OcrSession), OcrSettingsPanel.tsx (formatPct) | **ADR-081**: RegionSelector class — view/select/edit overlay on video parent. `render()` UPDATES nodes (label text in place; handles created once per edit entry with mousedown bound at creation, %-anchored CSS tracks resizes; toolbar rebuilt only on view↔interactive boundary) — never recreates interactive nodes, fixing dead resize handles after the first move drag. Styling via one injected `#cell-ocr-region-style` class sheet (`.cell-ocr-region-rect/label/handle/btn`) driven by `--overlay-ocr-region-*` tokens + `--radius-pill`; `data-mode` on rect drives cursor/fill; `formatPct()` rounds displayed %. `defaultBottomRegion(pct, widthPct)` centered-bottom region. Regression tests in regionSelector.test.ts |
...
| `features/ocr/ui/OcrSettingsPanel.tsx` | Toggle (shared/ui), ocrStateStore (loadOcrSettings, saveOcrSettings, getOcrPreference, clearOcrPreference, setOcrPreference, extractOriginFromUrl), ocrStateTypes | SubtitleManagerPanel.tsx (OCR tab) | **Orca T17-T20 + ADR-081**: Manager Panel OCR tab UI — per-origin toggle (icon scanText), language mode select (auto/zh/en/ja), subtitle region % sliders (icons moveVertical/moveHorizontal), Select Region (crop) / Edit (move, only when custom region exists) / Reset (rotateCcw) commands via OCR_REGION_COMMAND. Custom region readout rounded via formatPct. Persists to chrome.storage.local on change |
