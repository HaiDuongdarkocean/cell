...
│   │       └── ui/PlayerModeOverlay.tsx + .module.css  # Player Mode: fixed full-viewport overlay (VideoStage → ContentOther → PlayerActionDock); reuses SubtitleBlock + NavCluster; dock always visible; z-index --z-overlay-settings (below dictionary popup)
...
│   │       └── logic/playerModeGeometry.ts  # Player Mode pure geometry: togglePlayerMode, clampDockHeight, clampDictionarySheetHeight, resolvePlayerModeLayout (video/content/dock); O(1); unit tested
...
||| `features/dictionaryPopup/ui/usePopupPosition.ts` | popupGeometry (constants, types, computePopupPosition, clampPopupSize, finalizePosition), React | Dictionary | React hook: viewport-aware position, drag, resize, and bottom-sheet logic reused from popupGeometry pure helpers. Sheet handle: 1:1 drag = height (no translate, no tier snap), stop position = sheet height; click on handle (≤4px move) = close; drag past 20% vh (raw projected height) = close |
