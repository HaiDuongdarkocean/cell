...
│   │       └── ui/SubtitlePanels.tsx + .module.css  # T045: shared shadow-root container composing SubtitleBlock, NavCluster, SubtitleManagerPanel, SubtitleOffsetPanel, SubtitleToast, SubtitleHint; exposes imperative controller via ref; Player Mode toggle (player-mode-btn replaces generate-native-btn in secondaryCol; generate-native-btn moved to extraCol). **Child-iframe Player Mode uses native `document.documentElement.requestFullscreen()`; `fullscreenchange` syncs `playerMode`; `attachFullscreenReparenting` moves the shadow host into `document.fullscreenElement`; body reparenting is skipped for child frames.**
...
│   │       └── logic/iframePlayerModeBridge.ts  # Cross-origin Player Mode coordinator: child→top enter/exit messages, host `.player-wrap` discovery, reversible host reparenting, and child acknowledgement.
...
│   │       └── ui/subtitleShortcuts.ts  # Keyboard shortcut handler: handleShortcutKey (pure, guard input/textarea); ADR-027: supports 'generate-native' and 'play-pause' actions
...
│   │       └── logic/playerModeGeometry.ts  # Player Mode pure geometry: togglePlayerMode, resolveVideoAspectRatio, host/video style maps, clampDockHeight, clampDictionarySheetHeight, resolvePlayerModeLayout (video/content/dock); O(1); unit tested
...
│   │       └── logic/findPlayerContainer.ts + .test.ts  # **ADR-078**: Player container discovery — largest playable <video> → farthest same-size ancestor (15% tolerance). Used by PlayerModeOverlay to slot the full player container (with controls) into the shadow DOM video stage while keeping its CSS in light DOM.
