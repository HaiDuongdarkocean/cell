...
|| `features/dictionaryPopup/ui/useCandidate.ts` | runtime, messages, translation, wordStatusStore, wordStatusClient, buildCandidatePrefill, popupContent | CandidateView | Hook: per-candidate state (active tab, definition selection, lazy audio/image fetch, translation), tab selection-count badges, card-creator prefill |
|| `features/dictionaryPopup/ui/CandidateView.tsx` | shared/ui, icons, rankToBand, config, useCandidate, DictionaryPanelView.module.css | DictionaryPanelView | React card: candidate header, toolbar tabs, lazy media panels, definitions |
...
|| `features/dictionaryPopup/ui/DictionaryPanelView.tsx` | shared/ui, icons, rankToBand, config, runtime, messages, translation, CandidateView | DictionaryTab | React view: search input, history, candidate chip bar, scrollable CandidateView list |
