...
│   │       └── logic/iframeContext.ts  # **new**: child-iframe detection (`isChildFrame`); single source of truth for Player Mode branching between top-frame (reparent video into video stage) and child-iframe (native fullscreen + transparent overlay).
