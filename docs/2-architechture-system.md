...
| `TokenizeScheduler` | `features/tokenize/logic/tokenizeScheduler.ts` | class (priority queue + idle fallback) | webTokenizeController, subtitleTokenizeController | Viewport > buffer > idle priority tasks; budget-aware runLoop; `flush()` for tests |
...
| `findFrequencyByTerms` | `features/dictionary/repositories/frequencyRepository.ts` | (langCode, terms) → Promise<Map<string, FrequencyEntry[]>> | webTokenizeController, subtitleTokenizeController | Batch fetch frequency entries for many terms in one IDB transaction |
