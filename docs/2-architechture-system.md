...
│   └── universalPanel/  # Universal side panel (ADR-065) — types, UniversalPanelController, mountUniversalPanel, UniversalPanel (React shell), UniversalPanel.module.css, tabs/SettingsTab, SettingsTab.module.css, tabs/DictionaryTab (+DictionaryTab.module.css), tabs/CardCreatorPanel (+CardCreatorPanel.module.css); tabs/ use shared/ui hooks/patterns; Dictionary left pane uses dictionaryPopup/ui/useDictionaryPanel + DictionaryPanelView (+DictionaryPanelView.module.css) with search, definitions, candidate chips, audio/image/translate/links tabs, status cycle, and Send to Card; right pane CardCreatorPanel placeholder; right-edge slide-in up to 1280px; responsive bottom sheet on mobile; tab persistence via chrome.storage.session; Settings tab reuses SettingsDialogContent with tokenize bridge and initial-focus handoff
...
|| `features/universalPanel/mountUniversalPanel.ts` | UniversalPanel, UniversalPanelController, themeTokens, chrome-apis, config, settingsStore, dictionaryPopup/ui | content-script.ts | Mount fixed host + React root + theme sync + fullscreen reparenting; wires SettingsTab `onOpenDictionary` to `controller.open('dictionary')`; loads settings and renders `DictionaryTab` with `langCode/sourceLang/targetLang` props; returns controller |
...
|| `features/universalPanel/tabs/DictionaryTab.tsx` | DictionaryPanelView, CardCreatorPanel, dictionaryPopup types | mountUniversalPanel | Two-pane layout (left dictionary / right card-creator placeholder); manages `prefill` state and forwards `onSendToCard` |
|| `features/universalPanel/tabs/DictionaryTab.module.css` | tokens | DictionaryTab | Two-pane flex; mobile stack |
|| `features/universalPanel/tabs/CardCreatorPanel.tsx` | shared/ui, dictionaryPopup types | DictionaryTab | Placeholder right pane: shows prefill summary and `Open Card Creator` button |
|| `features/universalPanel/tabs/CardCreatorPanel.module.css` | tokens | CardCreatorPanel | Placeholder card styles |
...
|| `features/universalPanel/tabs/DictionaryTab.test.tsx` | DictionaryTab, mocks for runtime sendMessage/translation | — | Unit tests: left/right panes, initial search, Send to Card prefill, onSendToCard forward, status cycle, translate tab |
