...
│   └── dictionaryPopup/  # Popup Dictionary (spec §9, ADR-037) — types, schema; worker/lookupWorker, lookupWorkerHandler, workerFactory, phraseIndexLoader, resourcePriority; logic/lruCache, lookupOrchestrator, lookupOrchestratorBenchmark, englishLemma (ADR-041: unified multi-candidate lemma); trigger/subtitleTriggerController, subtitleTokenWrap, webTextTriggerController; services/wordStatusStore, quickAddAssembler, quickAddHandler, sendToCreator; ui/popupShell (ADR-042: 2-layer shell — outer [data-dp-popup] overflow:hidden + inner [data-dp-content] scroll wrapper, contentEl field, getContainer returns contentEl, reAppendResizeHandle no-op), popupContent (ADR-042: getOrCreateCandidateList wrapper div[data-dp-candidate-list], sticky data-dp-header), popupToolbar, popupDictionaryController; plugins/languagePlugin, englishPlugin, chinesePlugin, fallbackPlugin, pluginRegistry
...
| `SubtitleTriggerController` | `features/dictionaryPopup/trigger/subtitleTriggerController.ts` | class: attach/detach/setTriggerMode/cancelInFlight | contentScriptController | **spec §4.6.3 A2, §9.4, D9**: debounce 150ms hover / 50ms click, LOOKUP_CANCEL on new trigger, requestId routing; wraps subtitle text span into per-token spans (EN per-word, ZH per-segment) |
...
| `WebTextTriggerController` | `features/dictionaryPopup/trigger/webTextTriggerController.ts` | class: attach/detach/setTriggerMode/cancelInFlight | contentScriptController | **spec §5.2 P1**: web text lookup — mouseup with selection → dispatch LookupRequest; hover+modifier → keydown with selection; auto-detect sentence from closest block ancestor; ignores Shadow DOM targets (popup) |
| `PopupShell.getContainer` | `features/dictionaryPopup/ui/popupShell.ts` | () → HTMLDivElement \| null | popupDictionaryController, popupContent | **ADR-042**: Returns `contentEl` (inner scroll wrapper `[data-dp-content]`), NOT the outer shell `container`. Content renders here; resize handle is a sibling so clearing this element leaves the handle intact |
| `PopupShell.reAppendResizeHandle` | `features/dictionaryPopup/ui/popupShell.ts` | () → void | popupDictionaryController | **ADR-042**: No-op (backward compat). Resize handle is now a sibling of the scroll wrapper, not a child — no re-append needed after clearContainer |
| `getOrCreateCandidateList` | `features/dictionaryPopup/ui/popupContent.ts` | (HTMLElement) → HTMLElement | renderPopupContent, appendCandidateContent | **ADR-042**: Get or create `div[data-dp-candidate-list]` (`display:block`) inside container. Block-level wrapper avoids iOS Safari bug where position:sticky fails inside flex containers |
| `renderPopupContent` | `features/dictionaryPopup/ui/popupContent.ts` | (container, LookupResult, WordStatus, DefinitionSelection, CandidateCallbacks) → void | popupDictionaryController | **ADR-042**: Clears container → getOrCreateCandidateList → renderCandidate into wrapper. Each `data-dp-header` has `position:sticky;top:0;background:var(--dp-bg)` for iOS-style sticky section headers |
| `appendCandidateContent` | `features/dictionaryPopup/ui/popupContent.ts` | (container, LookupResult, WordStatus, DefinitionSelection, CandidateCallbacks) → HTMLElement | popupDictionaryController | **ADR-042**: Append candidate to existing `div[data-dp-candidate-list]` wrapper (progressive rendering of additional phrase match candidates) |
