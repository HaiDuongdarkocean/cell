...
|| `features/dictionaryPopup/ui/DictionaryPanelView.tsx` | shared/ui, useDictionaryPanel, searchHistory, CandidateView | Dictionary | React view: search input, bounded history, candidate chip bar with jump-scroll, scrollable CandidateView list (no global tab state); hides search row and history when `variant='popup'` |
