...
│   └── dictionaryPopup/  # Popup Dictionary (spec §9, ADR-037) — types, schema; worker/lookupWorker, lookupWorkerHandler, workerFactory, phraseIndexLoader; (logic/lruCache pending Task 1.3)
...
| `handleWorkerMessage` | `features/dictionaryPopup/worker/lookupWorkerHandler.ts` | (LookupWorkerState, WorkerRequestMessage) → WorkerLookupResultMessage[] | lookupWorker entry | **ADR-037 §8 + spec §9.4**: HYDRATE_CHUNK stores resident index; LOOKUP runs matchPhrase + word fallback; LOOKUP_CANCEL drops by requestId |
