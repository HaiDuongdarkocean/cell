...
|| `features/dictionaryPopup/ui/buildCandidatePrefill.ts` | types | useCandidate, useDictionaryPanel | Pure function: build PopupCardCreatorPrefill from candidate + selections |
...
||| `features/dictionaryPopup/ui/usePopupPosition.ts` | popupGeometry (constants, types, computePopupPosition, clampPopupSize, finalizePosition), React | PopupDictionary | React hook: viewport-aware position, drag, resize, and bottom-sheet logic reused from popupGeometry pure helpers |
...
| `mountCardCreatorDialog` | `features/cardCreator/ui/mountCardCreatorDialog.ts` | (settings, themeSource?) → CardCreatorMountController | contentScriptController.ts | **T082**: Mount Card Creator into full-viewport fixed shadow host via `mountReactShadow`; `USE_LEGACY_CARD_CREATOR` flag keeps old light-DOM path. Chooses desktop `Dialog` or mobile `BottomSheet` based on viewport width |
...
| `mountReactShadow` | `shared/lib/shadowRoot/mountReactShadow.ts` | ReactElement, options → {host, shadow, unmount} | ShadowThemeProvider, card creator, settings, universal panel | **T082/ADR-075**: Generic open-shadow root React mount; injects tokens + per-component CSS; optional `reparentOnFullscreen` keeps overlay visible in video fullscreen |
