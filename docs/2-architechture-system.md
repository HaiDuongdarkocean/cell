...
| `getOrCreateCandidateList` | `features/dictionaryPopup/ui/popupContent.ts` | (HTMLElement) → HTMLElement | renderPopupContent, appendCandidateContent | Get or create `div.js-cell-candidate-list` inside container; block wrapper avoids iOS Safari sticky flex bug |
| `renderPopupContent` | `features/dictionaryPopup/ui/popupContent.ts` | (container, LookupResult, WordStatus, DefinitionSelection, CandidateCallbacks) → void | popupDictionaryController | Clears container → getOrCreateCandidateList → renderCandidate. CandidateCallbacks now includes optional onClose/onPlayTerm |
| `appendCandidateContent` | `features/dictionaryPopup/ui/popupContent.ts` | (container, LookupResult, WordStatus, DefinitionSelection, CandidateCallbacks) → HTMLElement | popupDictionaryController | Append candidate to existing `div.js-cell-candidate-list` wrapper; progressive rendering of additional phrase match candidates |
| `renderHeader` | `features/dictionaryPopup/ui/popupContent.ts` | (container, result, status, onCycle, onQuickAdd, onSend, onSettings, onClose?, onPlayTerm?, isPrimary?) → void | renderCandidate, renderPopupContent | Term + reading + frequency + status badge; inline audio button; close/settings/send/quick-add icon buttons |
| `renderCandidate` | `features/dictionaryPopup/ui/popupContent.ts` | (container, result, status, selection, callbacks, isPrimary?) → HTMLElement | renderPopupContent, appendCandidateContent | Wraps header + toolbar slot + definitions; passes onClose/onPlayTerm when isPrimary |
...
| `playTermAudio` | `features/dictionaryPopup/ui/popupDictionaryController.ts` | (term, langCode, audioItems) → Promise<void> | popupDictionaryController | Header audio button: play first cached Forvo URL or system TTS |
