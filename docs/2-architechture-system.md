...
│   └── universalPanel/  # Universal side panel (ADR-065) — types, UniversalPanelController, mountUniversalPanel, UniversalPanel (React shell), UniversalPanel.module.css, tabs/SettingsTab, SettingsTab.module.css, tabs/DictionaryTab (+DictionaryTab.module.css), tabs/CardCreatorPanel (+CardCreatorPanel.module.css); tabs/ use shared/ui hooks/patterns; Dictionary left pane uses dictionaryPopup/ui/useDictionaryPanel (search, definitions selection, candidate active, audio/image/translate/links lazy fetch + selection) + DictionaryPanelView (+DictionaryPanelView.module.css) (audio Word/Sentence subtab, max 3 per tab, skeleton/error/empty states) with status cycle and Send to Card / Quick Add; right pane CardCreatorPanel renders CardCreatorDialogContent directly with prefill including selected definitions, audio URLs, and image URLs; right-edge slide-in up to 1280px; responsive bottom sheet on mobile; tab persistence via chrome.storage.session; Settings tab reuses SettingsDialogContent with tokenize bridge and initial-focus handoff
