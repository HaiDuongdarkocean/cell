...
│   │       └── ui/reactSubtitleController.ts + mountSubtitle.tsx + SubtitlePanels.tsx  # T046: React subtitle UI controller, mount helper, and panel composition; Player Mode temporarily top-aligns host video container and publishes document bounds for Dictionary sheet
...
│   │       └── ui/SubtitleManagerPanel.tsx + .module.css + .test.tsx  # T042: React subtitle manager panel (select/load/import, naming, offset entry) + Appearance view (Block/Target/Native/Cluster style customization, ADR subtitle-appearance-in-manager)
...
│   │       └── ui/SubtitlePanels.tsx + .module.css  # T045: shared shadow-root container composing SubtitleBlock, NavCluster, SubtitleManagerPanel, SubtitleOffsetPanel, SubtitleToast, SubtitleHint; exposes imperative controller via ref; Player Mode toggle and normal-page Split View toggle. Split View wraps the outer player shell so native controls + Cell overlay stay inside the video stage; CueList is rendered through the shared SubtitlePanel SSOT. CSS owns resizing: the CueList keeps its persisted flex percentage, the stage receives the remaining width, and viewport-bound player wrappers use `100dvh`; no descendant pixel-size chain or synthetic resize event is used. **Child-iframe Player Mode uses native `document.documentElement.requestFullscreen()`; `fullscreenchange` syncs `playerMode`; `attachFullscreenReparenting` moves the shadow host into `document.fullscreenElement`; body reparenting is skipped for child frames.**
...
│   │       └── ui/mountSubtitle.tsx + .test.tsx  # T039/T045: mounts SubtitlePanels into a single shared shadow root; injects manager/search + shared atom CSS; returns unmount + panel visibility/toast API
...
│   │       └── logic/findPlayerContainer.ts + .test.ts  # **ADR-078**: Player container discovery — largest playable <video> → farthest same-size ancestor (15% tolerance). the farthest-ancestor walk accepts a wider controls-owning shell within a safe area ratio, so Split View moves video + controls + Cell overlay as one shell; PlayerModeOverlay continues to slot the same player container into the shadow DOM video stage while keeping host CSS in light DOM.
...
│   │       └── ui/SubtitleSearchPanel.tsx + .module.css + .test.tsx  # **ADR-079**: Search UI (query input + lang select + season/episode + results list + cue preview + role picker Target/Native); debounce 300ms + AbortController; collapsed hint when no key
│   ├── settings/       #   Settings UI + validation logic; ui/ responsive redesign (ADR-062) + strict 14/12px type scale
│   │   └── ui/ApiKeyManager.tsx + .module.css + .test.tsx  # **ADR-079**: API key CRUD UI (add/edit/delete + list by provider SubDL/OpenSubtitles + status badge active/rate-limited/invalid/unverified + remaining downloads); masked key display (last 4 chars); delete confirm dialog
...
| `SubtitleManagerPanel` | `features/subtitle/ui/SubtitleManagerPanel.tsx` | (props) → ReactElement | SubtitlePanels | React manager panel; consumes `SubtitlePanelItem[]` from `subtitlePanelModel`. **ADR subtitle-appearance-in-manager**: now has 2 views ('tracks' | 'appearance'); appearance view renders Block/Target/Native/Cluster sections using `SubtitleStylePanel`, `SubtitleBlockSettingsPanel`, `NavClusterSettingsPanel` from `./appearance/`. **Latency redesign**: `OffsetStepper` redesigned per `latency-control-redesign.html` mockup — label "Latency" + `[-0.5s][value][+0.5s]` row + `Reset` full-width; no clamp limit (user can set any offset); `formatSigned` helper for số trần display (no "s" suffix); `SectionState.lastValid` tracks last valid offset for invalid-input restore. |
...
| `SubtitleSearchPanel` | `features/subtitle/ui/SubtitleSearchPanel.tsx` | (props) → JSX | SubtitleManagerPanel | **ADR-079**: Search UI — query input + lang select + season/episode + results list + cue preview + role picker (Target/Native); debounce 300ms + AbortController; collapsed hint when no key |
| `ApiKeyManager` | `features/settings/ui/ApiKeyManager.tsx` | (props) → JSX | SettingsDialogContent | **ADR-079**: API key CRUD UI — add/edit/delete + list by provider + status badge (active/rate-limited/invalid/unverified) + remaining downloads; masked key (last 4 chars); delete confirm dialog |
