...
│   │       └── logic/findPlayerContainer.ts + .test.ts  # **ADR-078**: Player container discovery — largest playable <video> → farthest ancestor whose bounding width AND height match the video (rounded to px, per-dimension exact match). Mirrors iframePlayerModeBridge.ts iframe variant. Rejects same-height-different-width containers (e.g. themoviebox .player-container 1635×690 vs video 1226×690); Split View moves video + Cell overlay as one shell; PlayerModeOverlay slots the same player container into the shadow DOM video stage while keeping host CSS in light DOM.
