...
│   │       └── ui/PlayerModeOverlay.tsx + .module.css  # Player Mode: fixed full-viewport overlay (VideoStage → ContentOther → PlayerActionDock); host player container is slotted from the shadow host's light DOM into video stage so controls + CSS stay intact; reuses SubtitleBlock + original two-column NavCluster/action cluster composition inside the dock; Escape exits via window listener; dock always visible
...
│   │       └── logic/findPlayerContainer.ts + .test.ts  # Player container discovery: largest playable <video> → farthest same-size ancestor (tolerance 15%). Used by PlayerModeOverlay to move the full player container (with controls) into the video stage.
