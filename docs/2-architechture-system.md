...
| `detectSubtitle` | `lib/detectors/subtitleDetector.ts` | Request → DetectedSubtitle \| null | background/index.ts | Detect subtitle from request (URL, MIME); passes `request.initiator` (iframe player origin) into `DetectedSubtitle.initiator` for CDN hotlink-protection Referer |
