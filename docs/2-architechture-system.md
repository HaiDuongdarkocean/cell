...
│   │   └── ui/contentScriptController.ts  # M20 + T046: subtitle UI orchestration (init → returns cleanup for SPA episode-switch re-init) — instantiates ReactSubtitleController and mounts the React shadow-root subtitle UI. **ADR-101**: owns window capture keyboard listeners and routes events using composedPath real target; Cell UI shortcuts win over host while focused
...
│   │       └── ui/subtitleShortcuts.ts  # Keyboard shortcut handler: handleShortcutKey (pure, guard input/textarea); supports Player Mode ('g'), generate-native ('h'), and play-pause actions. **ADR-101**: adds isEditableEvent (composedPath real target) and isActivatableTarget (button/link/role=button)
...
New files: `autoDiscovery.logic.ts` (pure taxonomy), `MissingShowcasePlaceholder.tsx/.module.css`, `vite.showcase.config.ts` build target outputs `dist/design-system-showcase/design-system-showcase.html`, `package.json` serves via `dist/design-system-showcase`.
...
│   ├── subtitleShortcuts.ts       # Keyboard shortcuts: handleShortcutKey (pure, guard input/textarea). **ADR-101**: isEditableEvent (composedPath) and isActivatableTarget
...
|| `features/dictionaryPopup/ui/buildCandidatePrefill.ts` | types | useCandidate | Pure function: build PopupCardCreatorPrefill from candidate + selections, including the PopupSelectionSnapshot (selectedImageIds/selectedAudioIds/selectedDefinitionIds/translationSelected) cloned into the integrated Dictionary |
...
| `shared/ui/Button.tsx` + `.module.css` + `.style-guard.test.ts` + `.showcase.tsx` + `.showcase.module.css` | — | App.redesigned, ResourceCard, Dropzone, ImportProgress, DeleteConfirmModal, ResourcesPanel, ThemePanel, ThemeImportExport, SelectionBar, SubtitleManagerFooter, OcrSettingsPanel | **Liquid Glass button atom (v4)**: 6 action variants (primary/glass/outline/ghost/success/destructive) + aliases (secondary/primarySubtle/link), all sharing the same smoked-blue liquid-glass material with caustic rim highlights. 4 sizes (sm/md/lg/xl), horizontal/vertical orientations, 8 states incl. active toggle/loading/error, pill shape. Pointer-down spawns a water ripple from the touch point (scale 0 → 2.8, opacity 0.6 → 0, 700ms ease-out) and scales the surface to 0.985 over 80ms; release transitions back over 180ms ease-out. Focus-visible uses a 2px `currentColor 60%` outline with 3px offset. Style guard enforces token contracts. |
...
| `check-icons.js` | `scripts/check-icons.js` | — | package.json prebuild | Icon QC: geometry, catalog↔file integrity, dead entries, empty tags, duplicate SVG detection |
| `check-design-system-css.mjs` | `scripts/check-design-system-css.mjs` | — | package.json | Parser-based CSS audit: hardcoded colors/spacing/radius/z-index and undefined tokens in `src/shared/ui/*.module.css` |
...
|| `PatternAsyncStates` | `entrypoints/design-system-showcase/pages/PatternAsyncStates.showcase.tsx` | `Showcase` + `showcaseMeta` | `ShowcaseGallery` | Pattern example: idle/loading/empty/error/success composed from Spinner, Skeleton, EmptyState, Alert, Card |
|| `PatternSearchResult` | `entrypoints/design-system-showcase/pages/PatternSearchResult.showcase.tsx` | `Showcase` + `showcaseMeta` | `ShowcaseGallery` | Pattern example: SearchField + Tabs filter + loading/empty/error/results composed from SearchField, Tabs, Card, EmptyState, Alert, Spinner, Skeleton |
|| `PatternFormSubmit` | `entrypoints/design-system-showcase/pages/PatternFormSubmit.showcase.tsx` | `Showcase` + `showcaseMeta` | `ShowcaseGallery` | Pattern example: InputField, Select, Button, Dialog, Alert for validation/loading/success/error/destructive confirmation |
|| `PatternVocabularyCapture` | `entrypoints/design-system-showcase/pages/PatternVocabularyCapture.showcase.tsx` | `Showcase` + `showcaseMeta` | `ShowcaseGallery` | Pattern example: clickable word tokens, Dialog lookup, Select status, StatusDot + Text for vocabulary list |
|| `PatternSubtitleAcquisition` | `entrypoints/design-system-showcase/pages/PatternSubtitleAcquisition.showcase.tsx` | `Showcase` + `showcaseMeta` | `ShowcaseGallery` | Pattern example: InputField, Button, Card, EmptyState, Alert, Spinner, StatusDot for detect/list/acquire subtitle flow |
...
|| `pattern E2E` | `e2e/showcase-patterns-async-states.spec.ts` | `test.describe('Pattern: Async states')` | `playwright.config.ts` (`showcase` project) | Five deterministic tests: loading/empty/error/success states, keyboard cycling, axe on each state |
|| `pattern E2E` | `e2e/showcase-patterns-search-result.spec.ts` | `test.describe('Pattern: Search → result')` | `playwright.config.ts` (`showcase` project) | Five deterministic tests: search results, tab filtering, empty/error states, keyboard Enter, axe per state |
|| `pattern E2E` | `e2e/showcase-patterns-form-submit.spec.ts` | `test.describe('Pattern: Form submit')` | `playwright.config.ts` (`showcase` project) | Five deterministic tests: validation, success, error, dialog cancel/confirm, axe per state |
|| `pattern E2E` | `e2e/showcase-patterns-subtitle-acquisition.spec.ts` | `test.describe('Pattern: Subtitle acquisition')` | `playwright.config.ts` (`showcase` project) | Five deterministic tests: URL validation, detect/list candidates, acquire, empty/error, keyboard, axe per state |
