...
| `lookupOrchestrator` | `features/dictionaryPopup/logic/lookupOrchestrator.ts` | (LookupRequest, deps?, signal?) → Promise<LookupResult> | lookupWorkerHandler | **spec §4.6.3/§9.4**: EN phrase match (ADR-037) → dict query; ZH FMM segment → dict query; fallback token → dict query; surface word wins with origin lemma fallback (displayTerm support in assembleLookupResult; e.g. hover "is" shows "is" with definitions from "be"); `lookupOrchestratorMulti` returns ordered candidates: phrase → surface → origin → other matched phrases; `LookupResult` carries `hoverTerm` and `originTerm` for header display; reads persisted `WordStatus` via `getWordStatus` so the popup status button reflects shortcut/token status changes; lemma fallback (ADR-041: multi-candidate inflectional morphology) |
...
| `renderHeader` | `features/dictionaryPopup/ui/popupContent.ts` | (container, result, status, onCycle, onQuickAdd, onClose?, onPlayTerm?, onPlaySentence?) → void | renderActiveEntry | 2-row header: row1=phrase surface (or word) + phrase meta (hover token → origin lemma) + reading-row(IPA+audio) + actions; reading-row wraps on narrow popups; row2=badges+status |
