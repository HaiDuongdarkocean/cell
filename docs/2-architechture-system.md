...
│   └── universalPanel/  # Universal side panel (ADR-065) — types, UniversalPanelController, mountUniversalPanel, UniversalPanel (React shell), UniversalPanel.module.css; Dictionary + Settings tabs; right-edge slide-in up to 1280px; responsive bottom sheet on mobile; tab persistence via chrome.storage.session
...
|| `features/universalPanel/mountUniversalPanel.ts` | UniversalPanel, UniversalPanelController, themeTokens, chrome-apis, config | content-script.ts | Mount fixed host + React root + theme sync + fullscreen reparenting; returns controller |
