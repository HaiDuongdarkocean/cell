...
│   └── tokenize/       # Tokenize on Media (ADR-047) — VDLT-Hybrid: types; logic/textTokenizer, logic/tokenizeBlock (boundary detection), logic/viewportTracker (IntersectionObserver wrapper), logic/tokenizeCache (LRU), logic/tokenizeScheduler (idle chunk); controller/webTokenizeController, controller/subtitleTokenizeController; services/tokenizeSettingsStore (per-domain enable state), services/tokenizeStateStore (selected terms + toggles); ui/tokenSpanRenderer (bind/unbind token spans), ui/tokenSpanCss (host-page token styles), ui/tokenBadge (FAB + mini panel), ui/tokenBadgeCss
