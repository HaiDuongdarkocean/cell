...
| `mountTokenizeFab` | `features/tokenize/ui/mountTokenizeFab.ts` | (options) → { destroy } | content-script.ts | Mount `TokenizeFab` into a shadow root host on `document.body`, re-render with `ShadowThemeProvider` for host `data-theme`, bridge a `TokenizeStateStore` (or adapter) to `TokenizeFab`, and re-parent the host on `fullscreenchange` so it survives video fullscreen. |
