...
| `englishLemma` | `features/dictionaryPopup/logic/englishLemma.ts` | (word: string) → string | englishPlugin.lemma, lookupOrchestrator (fallback) | **ADR-041 + contractions**: single-candidate wrapper for `englishLookupCandidates()[0]`; handles negative contractions + clitics |
