...
│   │       └── ui/SubtitlePanels.tsx + .module.css  # T045: shared shadow-root container composing SubtitleBlock, NavCluster, SubtitleManagerPanel, SubtitleOffsetPanel, SubtitleToast, SubtitleHint; exposes imperative controller via ref; Player Mode toggle and normal-page Split View toggle. Split View wraps the outer player shell so native controls + Cell overlay stay inside the video stage; CueList is rendered through the shared SubtitlePanel SSOT. **Child-iframe Player Mode uses native `document.documentElement.requestFullscreen()`; `fullscreenchange` syncs `playerMode`; `attachFullscreenReparenting` moves the shadow host into `document.fullscreenElement`; body reparenting is skipped for child frames.**
