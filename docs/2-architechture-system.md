...
| `features/subtitle/ui/contentScriptController.ts` | subtitleBlockController, subtitleManagerPanel, offsetController, subtitleShortcuts, subtitleAutoLoad, subtitleMerge, translatePrefill, settingsStore, types (SrtCue, BilingualCue, SubtitlePanelItem, Settings) | content-script.ts | **M20**: Main subtitle UI orchestration. init returns cleanup; wires AUTO_LOAD_SUBTITLES, manager panel selection, generate-native flow, keyboard shortcuts, offset. **ADR-046**: shares `WebTextDictionaryController` for popup + highlight + card creator on subtitle tokens. **ADR-027**: `translatedNativeSlot` virtual replacement in manager panel |
...
| `getWordStatus` | `features/dictionaryPopup/services/wordStatusStore.ts` | (langCode, term) → Promise<WordStatus> | lookupOrchestrator | **spec §D1**: IndexedDB read, returns 'unknown' if not stored |
| `setWordStatus` | `features/dictionaryPopup/services/wordStatusStore.ts` | (langCode, term, status) → Promise<void> | PopupFooter | **spec §D1**: upsert word status entry |
...
| `tokenizeTextBlock` | `features/tokenize/logic/textTokenizer.ts` | (text, langCode?) → Token[] | webTokenizeController | Split text into word/punctuation tokens with UTF-16 offsets; EN uses phraseMatcher tokenizeSentence, ZH uses FMM fallback |
| `resolveTokenMetadata` | `features/tokenize/logic/textTokenizer.ts` | (Token[], getStatus, getFrequencyBand) → Promise<void> | webTokenizeController | Fill `status`/`frequencyBand` for non-separator tokens in-place; dedups terms |
| `findTextBlocks` | `features/tokenize/logic/tokenizeBlock.ts` | (root, options?) → TokenBlock[] | webTokenizeController | Walk text nodes, skip forbidden tags/roles, build TokenBlock list (ponytail: does not merge adjacent inline text nodes) |
| `TokenizeScheduler` | `features/tokenize/logic/tokenizeScheduler.ts` | class (priority queue + idle fallback) | webTokenizeController | Viewport > buffer > idle priority tasks; budget-aware runLoop; `flush()` for tests |
...
| `createWebTokenizeController` | `features/tokenize/controller/webTokenizeController.ts` | (options) → Promise<WebTokenizeController> | content-script.ts | Orchestrate VDLT-Hybrid: scan blocks → viewport tracker → scheduler → cache → renderer → badge; batched status/frequency lookup; Popup Dictionary wiring via `onOpenDictionary` |
| `rankToBand` | `features/tokenize/utils/frequencyBand.ts` | (rank) → TokenFrequencyBand | webTokenizeController | Rank thresholds: ≤1000 high, ≤5000 medium, ≤20000 low, else none (ponytail: arbitrary cutoffs, tune against corpus) |
| `entriesToBand` | `features/tokenize/utils/frequencyBand.ts` | (FrequencyEntry[]) → TokenFrequencyBand | webTokenizeController | Pick best (lowest) rank across resources |
| `bindTokenBlock` | `features/tokenize/ui/tokenSpanRenderer.ts` | (TokenBlock, displayOptions) → void | webTokenizeController | Wrap tokens in spans, apply status/frequency classes, attach hover/click/Ctrl+click handlers, restore original text on unbind |
| `createTokenBadge` | `features/tokenize/ui/tokenBadge.ts` | (options) → TokenBadge | webTokenizeController | Floating FAB + Shadow DOM mini panel with enable/status/frequency toggles and dictionary button |
| `loadTokenizeSettings` | `features/tokenize/services/tokenizeSettingsStore.ts` | () → Promise<TokenizeSettings> | webTokenizeController | chrome.storage.local per-origin/URL enable state |
| `createTokenizeStateStore` | `features/tokenize/services/tokenizeStateStore.ts` | (options) → TokenizeStateStore | webTokenizeController | In-memory reactive store: enabled, showStatus, showFrequency, hoveredTerm, selectedTerms |
| `findFrequencyByTerms` | `features/dictionary/repositories/frequencyRepository.ts` | (langCode, terms) → Promise<Map<string, FrequencyEntry[]>> | webTokenizeController | Batch fetch frequency entries for many terms in one IDB transaction |
