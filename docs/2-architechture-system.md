...
│   │   └── ui/contentScriptController.ts  # M20 + T046: subtitle UI orchestration (init → returns cleanup for SPA episode-switch re-init) — USE_LEGACY_SUBTITLE flag in src/shared/config/config.ts routes to SubtitleBlockController (legacy DOM) or ReactSubtitleController + mountSubtitle (React shadow root)
│   │       └── ui/subtitleCueEngine.ts  # T046: logic-only cue/offset/video/time/repeat engine shared by subtitleBlockController and ReactSubtitleController
│   │       └── ui/subtitleBlock*.ts  # ADR-025: subtitleBlockController + subtitleBlockDom + subtitleBlockDrag + subtitleBlockScale + subtitleBlockCss (unified draggable block: target overlay + native overlay + nav cluster merged into single block); T005: subtitleBlockCss + subtitleUI now use color-mix() with --color-overlay-* / --color-info tokens, raw rgba/hsl removed; ADR-027: subtitleBlockDom adds generate-native button, subtitleBlockController exposes setGenerateNativeEnabled / onGenerateNative callback; ADR-027 redesign: right column = 2 sub-columns (primary: quick-add + edit-card + more-btn popover; secondary: update-current-card + generate-native + manager-icon slot), overflow buttons (panel-toggle + import-button) moved into more-popover, top-left toolbar + top-right panel-toggle removed from video surface
│   │       └── ui/reactSubtitleController.ts + mountSubtitle.tsx + SubtitlePanels.tsx  # T046: React subtitle UI controller, mount helper, and panel composition; replaces legacy block when USE_LEGACY_SUBTITLE is false
│   │       └── ui/subtitleManagerPanel.legacy.ts  # ADR-015/ADR-027: legacy DOM-based subtitle manager panel; retained until T046 deletion
│   │       └── ui/SubtitleManagerPanel.tsx + .module.css + .test.tsx  # T042: React subtitle manager panel (select/load/import, naming, offset entry); replaces legacy panel once verified
...
│   │       └── ui/SubtitleBlock.tsx + .module.css + .test.tsx  # T036: React subtitle block overlay; renders active target + native cues from useCuesStore with OverlayStyleConfig
│   │       └── ui/NavCluster.tsx + .module.css + .test.tsx  # T037: React nav cluster; prev/repeat/next, rewind/play/forward, collapse, no-sub states
│   │       └── ui/subtitleUI.ts  # Shared subtitle overlay visual helpers: buildTextShadow, hexToRgba, sanitizeFontFamily
│   │       └── ui/navCluster*.ts + NavCluster.module.css  # ADR-018/T004: navClusterActions + navClusterButton + navClusterKeyboard + navClusterIcons + navClusterCss (layout-only) + NavCluster.module.css (visual tokens). navClusterController + navClusterDom DELETED ADR-025 — merged into subtitleBlock*
│   │       └── ui/offsetController.ts  # ADR-019: OffsetController class — wires subtitleOffsetSection (legacy, nested trong manager panel) + lazy/committed state
│   │       └── ui/subtitleOffsetPanel.legacy.ts  # ADR-019: legacy offset section DOM factory; retained until T046 deletion
