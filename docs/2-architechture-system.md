...
│   │       └── ui/PlayerModeOverlay.tsx + .module.css  # **ADR-078** Player Mode: fixed full-viewport overlay (VideoStage → ContentOther → PlayerActionDock); host player container is slotted from the shadow host's light DOM into video stage so controls + CSS stay intact; reuses SubtitleBlock + original two-column NavCluster/action cluster composition inside the dock; Escape exits via window listener; dock always visible. **Child-iframe mode (AnimeKai/megaplay/vidnest): same overlay layout as top frame; `PlayerModeOverlay` mount effect projects the player container into the video stage within the same child document, sizes it responsively, and the `videoStage ::slotted([slot="cell-video"])` keeps `pointer-events: auto` so native controls remain clickable. The shadow host is managed by `attachFullscreenReparenting` and is not moved to `document.body`.**
