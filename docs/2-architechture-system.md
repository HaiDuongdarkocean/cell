...
│   └── tokenize/       # Tokenize on Media (ADR-047, ADR-050, ADR-051) — VDLT-Hybrid: types; logic/textTokenizer (+prepareTokenBlock synchronous tokenization), logic/tokenizeBlock (boundary detection + findTextBlocksInNodes for incremental SPA re-scans), logic/viewportTracker (IntersectionObserver wrapper), logic/tokenizeCache (LRU + touch + domMap cleanup on evict/delete; capacity tuned by navigator.deviceMemory), logic/tokenizeScheduler (viewport fast path with real 16ms budget + idle chunk + task error isolation); controller/webTokenizeController (lazy scan/observe only while enabled; viewport 200px buffer, incremental MutationObserver re-scan with queueMicrotask fast path + removedNodes/characterData handling, batched status/frequency metadata, selective eager tryBindVisible, dynamic cache capacity, stale-block eviction, early-bind then async rebind; orchestrates scheduler, cache, renderer, badge + Popup Dictionary wiring), controller/subtitleTokenizeController; services/tokenizeSettingsStore (per-domain enable state), services/tokenizeStateStore (selected terms + toggles); utils/frequencyBand (rank → high/medium/low/very-low/none, with very-low for all remaining positive ranks); ui/tokenSpanRenderer (defensive bind/unbind token spans for SPA re-render; showFrequency toggle controls frequency layer for all token statuses), ui/tokenSpanCss (host-page token styles), ui/tokenBadge (FAB + mini panel), ui/tokenBadgeCss
...
| `createWebTokenizeController` | `features/tokenize/controller/webTokenizeController.ts` | (options) → Promise<WebTokenizeController> | content-script.ts | Lazy scan/observe only while enabled; persisted enable waits for `window.load` + 500ms child-list DOM quiescence (HYDRATION_QUIET_MS) capped by MAX_ACTIVATION_DELAY_MS=3000ms hard deadline (ADR-048: heavy SPA never goes quiet); mutation re-scan accumulates `addedNodes` and uses `findTextBlocksInNodes` for incremental scans; fast path schedules `queueMicrotask` to bind new nodes immediately, with a 300ms trailing-debounce fallback capped by MAX_MUTATION_SCAN_DELAY_MS=1500ms; also observes `characterData` and `removedNodes` to re-tokenize or clean up re-rendered text; viewport buffer 200px; eager `tryBindVisible` only for small mutation batches (<=50 nodes), avoiding forced reflow on large batches; cache capacity is dynamic by `navigator.deviceMemory` (100/150/250); stale blocks whose source node is detached are evicted so text is not left untokenized; visible blocks are bound immediately after synchronous tokenization, then status/frequency metadata is fetched in one batched background request and rebound; `blocks` array is pruned when cache evicts; cancels pending observer/listener on disable/destroy; disable disconnects observer, unbinds and clears blocks; setting changes rebind visible blocks; Popup Dictionary wiring |
...
| `rankToBand` | `features/tokenize/utils/frequencyBand.ts` | (rank) → TokenFrequencyBand | webTokenizeController, subtitleTokenizeController | Rank thresholds: ≤1000 high, ≤5000 medium, ≤20000 low; all remaining positive finite ranks become very-low; only missing/invalid data returns none (ADR-051) |
| `entriesToBand` | `features/tokenize/utils/frequencyBand.ts` | (FrequencyEntry[]) → TokenFrequencyBand | webTokenizeController, subtitleTokenizeController | Pick best (lowest) rank across resources; falls back to 'very-low' when there are no entries so every token gets a band (ADR-051) |
