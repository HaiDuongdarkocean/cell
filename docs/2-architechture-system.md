...
│   │       └── logic/subtitleBlockDrag.ts  # ADR-025: pure drag-to-reposition logic — clampYOffset (0-95), snapYOffset (25/50/75 ±8), dragDeltaToYOffset (px delta → percent + clamp + snap); O(1)
...
||| `features/dictionaryPopup/ui/usePopupPosition.ts` | popupGeometry (constants, types, computePopupPosition, clampPopupSize, finalizePosition), React | Dictionary | React hook: viewport-aware position, drag, resize, and bottom-sheet logic reused from popupGeometry pure helpers |
...
|| `features/dictionaryPopup/ui/Dictionary.tsx` | shared/ui, icons, DictionaryPanelView, usePopupPosition, PopupDictionary.module.css | PopupDictionary, DictionaryTab | React dictionary shell with `variant: 'popup' | 'integrated'` — popup adds drag header, close, resize/sheet handle and mounts DictionaryPanelView; integrated renders DictionaryPanelView directly |
...
|| `features/dictionaryPopup/ui/DictionaryPanelView.tsx` | shared/ui, useDictionaryPanel, searchHistory, CandidateView | Dictionary | React view: search input, bounded history, candidate chip bar with jump-scroll, scrollable CandidateView list (no global tab state); used by both popup and integrated variants |
...
| `clampYOffset` / `snapYOffset` / `dragDeltaToYOffset` | `features/subtitle/logic/subtitleBlockDrag.ts` | (value) → number / (startOffset, deltaY, containerHeight) → number | SubtitlePanels.tsx | **ADR-025 (React)**: Pure drag-to-reposition logic — clamp 0-95, snap 25/50/75 ±8, px delta → percent. O(1). Unit tested. |
