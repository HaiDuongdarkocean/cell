...
│   │       └── ui/subtitleBlock*.ts  # ADR-025: subtitleBlockController + subtitleBlockDom + subtitleBlockDrag + subtitleBlockScale + subtitleBlockCss (unified draggable block: target overlay + native overlay + nav cluster merged into single block); ADR-027: subtitleBlockDom adds generate-native button, subtitleBlockController exposes setGenerateNativeEnabled / onGenerateNative callback; ADR-027 redesign: right column = 2 sub-columns (primary: quick-add + edit-card + more-btn popover; secondary: update-current-card + generate-native + manager-icon slot), overflow buttons (panel-toggle + import-button) moved into more-popover, top-left toolbar + top-right panel-toggle removed from video surface
│   │       └── ui/subtitleManagerPanel.ts  # ADR-015/ADR-027: unified subtitle manager panel (auto + imported + translated virtual entry) — createSubtitleManagerPanel (no toolbar, returns icon for slot), updateTarget, updateNative, destroy; overlay icon is host-CSS-resistant and active state changes SVG color only
...
│   ├── lib/            #   parsers/, storage/, chrome-apis/ (adapters), themeTokens, tokens, frequencyBand
...
│   ├── subtitlePanel.ts           # Toggle button + seek helper: createToggleButton (ADR-027: returns element, no longer appends to container — caller appends to subtitle block more-popover slot; opens Side Panel; host CSS cannot restore its border), seekToCue — ADR-008. **ADR-030**: seekToCue routes qua seekVideo (Netflix M7375 fix)
...
| `features/subtitle/ui/contentScriptController.ts` | subtitleBlockController, subtitleManagerPanel, offsetController, subtitleShortcuts, subtitleAutoLoad, subtitleMerge, translatePrefill, settingsStore, tokenizeSettingsStore, types (SrtCue, BilingualCue, SubtitlePanelItem, Settings, TokenizeSettings) | content-script.ts | **M20**: Main subtitle UI orchestration. init returns cleanup; wires AUTO_LOAD_SUBTITLES, manager panel selection, generate-native flow, keyboard shortcuts, offset. **ADR-046**: shares `WebTextDictionaryController` for popup + highlight + card creator on subtitle tokens. **ADR-027**: `translatedNativeSlot` virtual replacement in manager panel; redesign — `createToggleButton()` returns element (no container append), `createSubtitleManagerPanel(container, opts)` (no importButton arg), `blockController.attachOverflowButtons({panelToggle, importButton, managerIcon})` moves overflow buttons into subtitle block slots, removes top-left toolbar + top-right panel-toggle from video surface. Real-time tokenize enable/disable via `syncSubtitleTokenize` reacting to `tokenizeSettings` storage changes |
| `features/subtitle/ui/subtitleManagerPanel.ts` | types (SubtitlePanelItem) | contentScriptController.ts | **ADR-015/ADR-027**: Unified manager panel rendering auto/imported/translated entries with source badges. updateTarget, updateNative, open/close/destroy, onSelect callback. **ADR-027**: no toolbar — returns `icon` for caller to append into subtitle block secondary column slot |
...
|| File | Import từ | Được import bởi | Sửa file này → ảnh hưởng |
||------|-----------|-----------------|--------------------------|
|| `features/universalPanel/types.ts` | — | UniversalPanel, UniversalPanelController, mountUniversalPanel | `UniversalPanelTab`, `DictionaryPanelPrefill`, `UniversalPanelController` interface |
|| `features/universalPanel/UniversalPanelController.ts` | types, chrome storage | mountUniversalPanel | `createUniversalPanelController`: imperative open/close/switchTab + session persistence |
|| `features/universalPanel/UniversalPanel.tsx` | types, Icon, IconButton, useFocusTrap | mountUniversalPanel | React shell: vertical tab bar, backdrop, X close, Escape, focus trap, responsive bottom sheet |
|| `features/universalPanel/UniversalPanel.module.css` | tokens | UniversalPanel | Slide-in panel styles: 1280px max-width, mobile bottom sheet, animations |
|| `features/universalPanel/mountUniversalPanel.ts` | UniversalPanel, UniversalPanelController, themeTokens, chrome-apis, config, settingsStore | content-script.ts | Mount fixed host + React root + theme sync + fullscreen reparenting; wires SettingsTab `onOpenDictionary` to `controller.open('dictionary')`; loads settings and renders `DictionaryTab` with `langCode/sourceLang/targetLang` props; returns controller |
|| `features/universalPanel/tabs/SettingsTab.tsx` | SettingsDialogContent, settingsStore, chrome-apis, config, entities/media, TokenizeSettingsPanel | mountUniversalPanel | Loads/saves settings, syncs via `chrome.storage.onChanged`, bridges tokenize state, auto-focuses first non-sidebar control |
...
|| `features/universalPanel/tabs/DictionaryTab.tsx` | DictionaryPanelView, CardCreatorPanel, dictionaryPopup types | mountUniversalPanel | Two-pane layout (left dictionary / right card creator); manages `prefill` state and passes `sourceLang`/`targetLang` to `CardCreatorPanel` |
|| `features/universalPanel/tabs/DictionaryTab.module.css` | tokens | DictionaryTab | Two-pane flex; mobile stack |
|| `features/universalPanel/tabs/CardCreatorPanel.tsx` | settingsStore, chrome-apis, config, CardCreatorDialogContent, useCardCreatorState, webTextDictionaryController/formatDefinitions | DictionaryTab | Loads Card Creator settings; builds `OpenContext` from prefill; renders `CardCreatorDialogContent` directly inside the panel |
|| `features/universalPanel/tabs/CardCreatorPanel.module.css` | tokens | CardCreatorPanel | Right pane flex layout: full height, overflow scrolling, subtle left border |
|| `features/universalPanel/UniversalPanel.test.tsx` | UniversalPanel, UniversalPanelController, testing-library | — | Unit tests: open/close, tab switch, persistence, backdrop/Escape close |
|| `features/universalPanel/tabs/SettingsTab.test.tsx` | SettingsTab, mocks for SettingsDialogContent/settingsStore/chrome-apis | — | Unit tests: load/save/sync, tokenize bridge, onOpenDictionary callback, initial focus |
|| `features/universalPanel/mountUniversalPanel.test.tsx` | mountUniversalPanel, UniversalPanelController, mocks for SettingsDialogContent/settingsStore/chrome-apis | — | Integration test: `onOpenDictionary` switches from Settings tab to Dictionary tab |
|| `features/universalPanel/tabs/DictionaryTab.test.tsx` | DictionaryTab, mocks for runtime sendMessage/translation/CardCreatorPanel | — | Unit tests: left/right panes, initial search, Send to Card prefill, status cycle, translate tab |
...
|| `features/dictionaryPopup/ui/useDictionaryPanel.ts` | runtime, messages, translation, wordStatusStore | DictionaryPanelView | Hook: LOOKUP_REQUEST/LOOKUP_CANCEL, currentResult/candidates, status cycle, translation, sendToCard prefill |
...
|| `features/dictionaryPopup/ui/DictionaryPanelView.tsx` | shared/ui, icons, rankToBand, config, runtime, messages, translation | DictionaryTab | React view: search input, header, definitions, candidate chips, audio/image/translate/links tabs, status + Send to Card footer |
|| `features/dictionaryPopup/ui/DictionaryPanelView.module.css` | tokens | DictionaryPanelView | Panel layout, header, definitions, tabs, footer styles |
...
| `seedDevDataIfEmpty` | `features/dictionary/logic/devSeed.ts` | (langCode?) → Promise<void> | background onInstalled, OptionsApp mount | Auto-seed: if DB empty, fetch Cambridge + frequency from extension bundle (dist/seed/), importFile in parallel; fire-and-forget; runs in both dev and production |
...
|| `mountUniversalPanel` | `features/universalPanel/mountUniversalPanel.ts` | (options) → UniversalPanelMountController | content-script.ts | **ADR-065**: fixed full-viewport host, createRoot, theme token injection, fullscreen re-parenting; wires `UniversalPanelController` to React; returns controller consumed by the orbital badge |
|| `createUniversalPanelController` | `features/universalPanel/UniversalPanelController.ts` | (options) → { controller, unmount } | mountUniversalPanel | **ADR-065**: imperative open/close/switchTab, session tab persistence, `getHosts` for click-outside handling |
...
| `injectTokenSpanStyle` | `features/tokenize/ui/tokenSpanCss.ts` | () → void | tokenSpanRenderer | Inject the token span stylesheet (buildTokenSpanCss) into the page head once. CSS uses `white-space: inherit !important` (not `normal`) so token spans respect the host page's white-space (e.g. nowrap nav links) instead of forcing wrapping; `overflow-wrap: break-word` only breaks long words that overflow. Status bar via inset box-shadow + box-decoration-break:clone for multi-line fragments. Frequency bands as solid pills with DS token colors. |
