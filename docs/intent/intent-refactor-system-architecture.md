...
> **Date**: 2026-06-30 (initial), 2026-07-01 (updated with architecture review findings).
> **Status**: Research complete + architecture review complete — chờ anh confirm intent trước khi vào G1 Spec.

---

## 0. Architecture Review Findings (2026-07-01)

> **Source**: `docs/reviews/architecture-review-2026-07-01.md` (generated by `chrome-extension-mv3-architecture-review` skill).
> **Context**: Refactor M0-M13 đã hoàn thành (FSD structure, 24 barrels, message passing healthy, no circular deps, features clean). Đây là findings của phần CÒN LẠI sau refactor.

### Checklist score: 17/42 failed

| Category | Failed | Severity |
|---|---|---|
| Lifecycle | 4/5 | **Critical — systemic** |
| Boundary | 3/6 | High |
| Storage | 3/5 | High |
| Injection | 3/5 | High |
| Message | 0/5 | **Pass — healthy** ✅ |
| Discoverability | 2/6 | Medium |
| Testability | 1/5 | Medium |
| Build/Config | 1/5 | Medium |

### Critical findings (3 — all Lifecycle, systemic)

| ID | Finding | Evidence |
|---|---|---|
| C1 | Background SW god-file (2,203 lines) doing heavy orchestration | `src/entrypoints/background/index.ts:1-2203` — 7+ responsibilities |
| C2 | SW performs `fetch()` directly — vulnerable to idle eviction | `index.ts:808, 1645, 1743` |
| C3 | No `onStartup`/`onInstalled` state rehydration | 0 handlers — `let backgroundService: BackgroundService \| null = null` (line 2160) assumes SW persistence |

### High findings (5)

| ID | Finding | Evidence |
|---|---|---|
| H1 | Entrypoints bypass chrome.* adapter (168 direct vs 21 adapter calls) | 12 entrypoint files call chrome.* directly |
| H2 | 36 deep imports bypass barrels | content-script.ts alone has 12 deep imports into features/subtitle |
| H3 | Strangler Fig stalled — 78 imports still use `@/types/` legacy barrels | `SubtitleFormat` diverged between two definitions |
| H4 | Content script heavy (774 lines, 12 deep imports) | Runs on every page via `<all_urls>` |
| H5 | No storage schema versioning | 0 `schemaVersion`/`migration` — settings break on schema changes |

### What's already healthy (keep, don't refactor)

- ✅ FSD structure sound — 7 top-level folders, 24 barrels, no god-folders
- ✅ Message passing excellent — typed `MessageType` union, `tabId` in all payloads, `Map`-based handler registry
- ✅ No circular dependencies — acyclic graph
- ✅ Features layer clean — 0 chrome.* calls in features/entities
- ✅ Test structure mirrors src + 4 E2E specs exist
- ✅ OPFS used for blobs — no quota risk
- ✅ CSP tight — only `wasm-unsafe-eval` for ffmpeg
...
> **Update 2026-07-01**: 2 "smell" gốc (subtitle*.ts phẳng, parallel*.ts phẳng) ĐÃ refactor qua M6-M7. Architecture review 2026-07-01 cho thấy debt còn lại tập trung vào **Lifecycle (SW god-file)** + **Boundary (deep imports, adapter bypass, Strangler Fig stalled)** — không còn folder structure smell.

Đọc `docs/2-architechture-system.md` + architecture review 2026-07-01 thấy debt còn lại:

1. **`src/entrypoints/background/index.ts` là god-file (2,203 lines)** — 7+ responsibilities (downloader instance, fetch calls, message routing, download queue, auto-download, settings sync). Vi phạm Screaming Architecture ở file level, không phải folder level.
2. **Entrypoints bypass chrome.* adapter (168 direct calls vs 21 adapter calls)** — adapter layer `shared/lib/chrome-apis/` tồn tại nhưng underused. Không testable entrypoints không có chrome global mock.
3. **36 deep imports bypass barrels** — consumers reach vào internal files thay vì qua `index.ts`. Feature boundaries leaky.
4. **Strangler Fig stalled — 78 imports still use `@/types/` legacy barrels** — 2 parallel type systems (`@/types/` + `@/entities/`), `SubtitleFormat` diverged.
5. **Content script heavy (774 lines, 12 deep imports)** — runs on every page via `<all_urls>`.
6. **No storage schema versioning** — settings break on schema changes, no migration path.

Đây là dấu hiệu **debt tập trung vào Lifecycle + Boundary**, không phải folder structure (đã healthy).
...
Trước khi vào G1 Spec, em cần anh confirm 4 câu hỏi dưới đây. Em sẽ hỏi qua `ask_user_question` tool ngay sau khi update file này.

### Q1. Phạm vi refactor phase tiếp theo

Architecture review cho thấy 3 Critical (Lifecycle) + 5 High (Boundary/Storage/Injection). Anh muốn ưu tiên nhóm nào trước?

- **Option A — Critical Lifecycle trước (P0)**: Split SW god-file (C1) + move fetch to offscreen (C2) + add onStartup/onInstalled (C3). 3 findings, cao nhất severity, fix user-visible bug "stuck downloads after SW restart". Effort: L+M+M.
- **Option B — High Boundary trước**: Route entrypoints through adapter (H1) + enforce barrel-only imports (H2) + finish Strangler Fig (H3). 3 findings, fix testability + discoverability. Effort: M+S+M.
- **Option C — Tất cả Critical + High**: Refactor cả 8 findings (C1-C3 + H1-H5). Phạm vi rộng nhất, 1 spec cover hết. Effort: ~5L.
- **Option D — Chỉ Critical, High để sau**: Fix 3 Critical Lifecycle, defer 5 High sang phase sau. Rủi ro thấp nhất, nhanh thấy kết quả.

### Q2. Target structure — giữ FSD hiện tại hay adjust?

- **Option A — Giữ FSD hiện tại**: `entrypoints/ + features/ + entities/ + shared/ + stores/ + app/ + types/`. Refactor focus vào internal discipline (barrel enforcement, adapter routing), không đổi top-level structure.
- **Option B — FSD + xóa `types/` + populate `app/`+`stores/`**: Hoàn thiện FSD — finish Strangler Fig (xóa `src/types/` sau khi migrate 78 imports), populate `src/app/` (global config) + `src/stores/` (move popupStore + sidePanelStore ra khỏi entrypoints).
- **Option C — Restructure entrypoints**: Tách `background/index.ts` thành `background/{index,handlers,orchestrator}/` sub-folders. Phạm vi hẹp hơn, focus vào god-file.

### Q3. Cadence commit

- **Option A — Commit nhỏ mỗi finding**: Mỗi finding (C1, C2, C3, H1...) = 1-3 commits. Dễ rollback, dễ review, test pass sau mỗi commit.
- **Option B — Commit mỗi milestone**: Group findings thành milestones (M14: SW split, M15: adapter routing, M16: Strangler Fig finish...). 1 commit per milestone.
- **Option C — 1 PR lớn**: Refactor trên branch riêng, 1 PR merge cuối. Ít commit noise nhưng review khó.

### Q4. Next step

- **Option A — Dừng G0, chờ anh review**: Anh đọc research + findings + câu hỏi này trước, confirm/điều chỉnh, rồi em mới vào G1 Spec.
- **Option B — Viết luôn G1 Spec**: Em viết luôn spec refactor dựa trên research + câu trả lời Q1-Q3 ở trên.

---

## 10. Answers (G0 → G1 gate — 2026-07-01)

| Question | Answer | Implication |
|---|---|---|
| Q1. Phạm vi | **Tất cả Critical + High** (8 findings: C1-C3 + H1-H5) | 1 spec cover hết, 8 findings refactor |
| Q2. Target structure | **Giữ FSD hiện tại** | Focus internal discipline (barrel enforcement, adapter routing), không đổi top-level |
| Q3. Cadence commit | **Commit nhỏ mỗi finding** | 1-3 commits/finding, test pass sau mỗi commit |
| Q4. Next step | **Viết luôn G1 Spec** | Em vào G1 ngay sau G0 commit |

---

## 11. Feasibility go/no-go (cuối G0)

### Build-vs-buy (lightweight)

| Option | Verdict |
|---|---|
| Build (refactor) | ✅ Chọn — codebase đã có test, FSD structure, adapter layer. Refactor incrementally. |
| Buy (rewrite from scratch) | ❌ Spolsky score 53/100 → hybrid refactor, không rewrite. Codebase đang chạy + có test. |
| Defer (không refactor) | ❌ 3 Critical Lifecycle = user-visible bug "stuck downloads after SW restart". Không defer được. |

### Risk thô

| Risk | Level | Mitigation |
|---|---|---|
| SW split break message routing | Medium | messageBus đã có typed handler registry — extract handlers ra modules, giữ messageBus wiring |
| Fetch move to offscreen break download flow | Medium | Offscreen đã tồn tại cho ffmpeg — pattern established, extend cho fetch |
| Strangler Fig finish break 78 imports | Low | `@/types/*` đã re-export từ `@/entities/*` — flip là mechanical, tsc catch error |
| Adapter routing break entrypoints | Low | Adapter đã có 4 APIs, extend thêm 4 — entrypoints thay import path, behavior giữ nguyên |

### Recommendation

**GO** — refactor 8 findings (C1-C3 + H1-H5). 3 Critical fix user-visible bug, 5 High fix maintainability + testability. Risk medium-low vì codebase đã có test + FSD + adapter layer. Effort ~5L nhưng chia nhỏ thành commit per finding → mỗi commit an toàn.
