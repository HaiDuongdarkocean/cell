...
| moviepire.ru | `https://moviepire.ru/watch/125988?s=1&e=2&me=10` | multi-provider embed (select switches iframe: videasy, vidnest, etc.) | ✅ Provider `Nest` → `vidnest.fun` native `<track>` subtitles now discovered; `PageScanner` runs in video iframes |
...
| moviepire/vidnest (`Nest`) | `vidnest.fun/tv/<tmdbId>/<s>/<e>` iframe with native `<video>` + `<track>` | **44 direct VTT** (e.g. `https://cache.vdrk.site/v3/tv/125988/1/2/English%20-%20English.vtt`). | ✅ `PageScanner` now scans video iframes; `<track>` URLs captured and delivered |
