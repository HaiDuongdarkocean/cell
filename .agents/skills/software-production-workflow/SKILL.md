...
**Skill activation**: `system-architecture-design` → `cto-persona` (governance) → `api-and-interface-design` (module boundaries) → `security-and-hardening` (threat model) → `frontend-ui-engineering` (UI design) → `conceptualization` (trigger 3: architecture decision → principle)
**Input**: `docs/specs/spec-<feature>.md`, `docs/plan/plan-<feature>.md` (approach/risk already there, ADR formalizes decision)
**Output**: `docs/adr/NNN-<decision>.md` (1 file per decision, NNN = sequence number), updated `docs/2-architechture-system.md`
**File ops**:
