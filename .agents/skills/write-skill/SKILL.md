...
10. **Self-evolution** (optional) — `self-evolution/` folder containing `README.md`, `workflow.md`, `RUNBOOK.md`, `mutation_prompts.md`, `test_cases.md`, and `archive/` for skills that self-correct.
11. **README.md** (optional) — human-facing usage guide; the agent ignores it unless asked.
12. **Router boomerang** — pointer back to `using-agent-skills`.
...
- When a new skill is written or an existing skill is updated, ask: *"Should this skill self-evolve from its own usage?"* If yes, proceed; if no, stop.
- After every run, if the skill has a `self-evolution/` folder, append one line to `self-evolution/RUNBOOK.md` (use this skill's `self-evolution/RUNBOOK.md` if the target is itself).
- After appending, read the trigger conditions in `self-evolution/workflow.md`; if they are met, run the self-correction loop.
- If the user explicitly asks for self-correction, run the loop immediately regardless of trigger conditions.
- When enabling self-correction, create `self-evolution/README.md` (human-readable, no frontmatter, not loaded by agent), `self-evolution/workflow.md`, `self-evolution/RUNBOOK.md`, `self-evolution/mutation_prompts.md`, `self-evolution/test_cases.md`, and `self-evolution/archive/README.md` in the target skill directory.
- Generate candidate mutations only from `self-evolution/mutation_prompts.md`; score them against `self-evolution/test_cases.md` and recent `self-evolution/RUNBOOK.md` entries.
- Replace `SKILL.md` only if the best candidate scores higher by at least 0.05 or fixes a critical failure.
- Archive the current version to `self-evolution/archive/` before overwriting and run regression tests immediately after.
...
| Evolving | If self-correction is claimed, it has a `self-evolution/` folder with README, workflow, RUNBOOK, mutation_prompts, test_cases, archive, and a rollback path. |
...
- [ ] If self-evolution is enabled, a `self-evolution/` folder exists with `README.md`, `workflow.md`, `RUNBOOK.md`, `mutation_prompts.md`, `test_cases.md`, and `archive/`.
