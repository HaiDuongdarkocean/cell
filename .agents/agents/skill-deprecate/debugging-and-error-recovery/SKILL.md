...
## Lesson: Behavior contract before code

A user request, feature idea, or bug report is not a plan. Do not start coding until you have turned it into an observable behavior contract that you can verify without asking the user.

### Universal lock protocol

For any behavior change — new feature, refactor, or bug fix:

1. **State the contract in one sentence**
   "When the user does X in state Y, the system shows Z."

2. **Expand into concrete Given/When/Then**
   - **Given**: exact starting state (URL, visible UI, persisted data, DOM element).
   - **When**: exact user or system action (hover, keypress, click, reload, API response).
   - **Then**: exact observable result (text content, class name, network call, popup visibility).

3. **Run internal Socratic triage**
   - What does "works" mean? What is the smallest visible proof?
   - What are the 3 most likely ways this could work for one case and fail for another?
   - Which layer owns the state? Which layer renders it? Where can they desync?
   - If my first theory is wrong, what single observation would prove it?

4. **Generate the variant matrix**
   Replay the contract while changing one variable at a time:
   - **Position**: first item, middle item, last item of a sequence.
   - **Context**: empty state, cached state, after reload, after error.
   - **UI state**: popup open/closed, selection active/inactive, badge visible/hidden.
   - **Data shape**: phrase vs. word, lemma vs. surface form, known vs. unknown status.

5. **Proceed only if you can explain the failure modes**
   - Can you name three specific inputs that would break your first implementation?
   - Can you explain why each variant could fail without using "maybe" or "probably"?

6. **Stop gates**
   - If you cannot write the contract, invoke `/interview-me` and self-interrogate.
   - If you cannot generate variants, the request is still too vague — stop and investigate.
   - If any variant fails, the task is not done. Do not declare pass.

The rule is not "ask the user more". The rule is "do the user's reasoning yourself before touching code."
