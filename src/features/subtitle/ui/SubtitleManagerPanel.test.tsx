...
describe('SubtitleManagerPanel', () => {
  ...
  it('disables -0.5s button at -60s clamp boundary', () => {
    const onOffsetChange = jest.fn();
    ...
    // Set offset to -60s via input
    const input = screen.getByTestId('manager-offset-input-target');
    fireEvent.change(input, { target: { value: '-60' } });
    fireEvent.blur(input);
    expect(onOffsetChange).toHaveBeenLastCalledWith('target', -60000);

    // Re-render to reflect internal state update
    rerender(
      <SubtitleManagerPanel
        targetItems={targetItems}
        nativeItems={nativeItems}
    ...
    const decBtn = screen.getByTestId('manager-offset-dec-target');
  ...
  it('disables +0.5s button at +60s clamp boundary', () => {
    ...
    const incBtn = screen.getByTestId('manager-offset-inc-target');
