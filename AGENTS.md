...
Any change touching content-scripts, popup UI, DOM injection, or extension runtime behavior **must be verified in a real browser** (MCP `chrome-devtools`/`edge-devtools`, or Playwright) before committing. Unit tests and `tsc` are necessary but not sufficient for visual/runtime bugs. No commit until the browser-level check passes. **Auto-activate `debugging-with-edge-devtools` skill** when: debugging UI/layout/fullscreen/content-script bugs, need live DOM measurement, need screenshot verification, or verifying acceptance criteria (C1-Cn) on a real video page. The skill encodes the `install_extension` MCP method (Chrome 149+ blocks `--load-extension` flag), DataTransfer drop simulation, and service-worker `chrome.storage` access patterns.
