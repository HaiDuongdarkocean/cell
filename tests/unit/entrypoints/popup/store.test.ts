...
describe('usePopupStore', () => {
  ...
  it('updateSettings merges partial settings and persists to chrome.storage.local', async () => {
    ...
    // saveSettings stamps schemaVersion (ADR-017 D8 / ADR-018 D2 / V4 / V5 / V6 / V7 / V8 / V9 / V10 / V11 / V12 / V13) — the persisted
    // payload includes schemaVersion: 13 in addition to the merged settings.
    expect(arg[STORAGE_KEYS.SETTINGS]).toEqual({ ...settings, schemaVersion: 13 });
  });
