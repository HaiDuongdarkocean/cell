...
describe('Background integration', () => {
  ...
  it('GET_SETTINGS returns settings from storage', async () => {
    ...
    expect(response.success).toBe(true);
    // loadSettings() runs migration v0→v1→v2→v3→v4→v5→v6 which stamps schemaVersion: 6
    // (ADR-017 D8, ADR-018 D2, ADR-019, V4 overlay defaults, V5 theme/buttonSize, V6 ASR toggle).
    expect(response.data).toEqual({ ...storedSettings, schemaVersion: 6 });
  });
